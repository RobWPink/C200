...
void daughterPrint(unsigned long dly){
  ...
  if(millis() - twoTimer > 2000 && twoTimer){
    ...
    sprintf(ln1,"C%02dCT%02dHT%02dMX%03d", CPMhigh, (int)AI_CLT_C_TT207_CoolantSupply2, (int)AI_HYD_C_TT454_HydraulicTank, (int)PTdata[1].max/100);
    switch(STATE){
      case FAULT:
...
float slope(vals a){
  ...
  return ((a.size * sum_xy - sum_x * sum_y) / (a.size * sum_x2 - sum_x * sum_x))*10;
}
  ...
  if(millis() - dataTimer > ptInterval){
    for(int i = 0; i < PTsize;i++){
      ...
      if(PTdata[i].max != -1 && PTdata[i].maxPause != -1){
        ...
        else if(*PTdata[i].value <= PTdata[i].maxRecovery && PTdata[i].overPressure && millis() - PTdata[i].overPressure > 10000){
      ...
      if(PTdata[i].min != -1 && PTdata[i].minPause != 1){
        ...
        else if(*PTdata[i].value > PTdata[i].minRecovery && PTdata[i].underPressure && millis() - PTdata[i].underPressure > 10000){
